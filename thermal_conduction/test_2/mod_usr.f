...
contains
  ...
  subroutine usr_init()
      ...
      unit_length = 1/1d8
      unit_numberdensity = 1/1d9
      unit_temperature = 1/974204.9765757825
      unit_velocity = 1/1.5d7
  ...
  subroutine rm1d_init_one_grid(ixGmin1,ixGmax1,ixmin1,ixmax1,w,x)
    ...
    double precision :: M, rho_eq, v_0, v_eq, p_eq, rho_1, v_1, v_sh, p_1, M1,&
      cs_0
    double precision :: q_th, rho_th, x_th_ini, x_th_fin, x_sh, thick_sh
    ...
    elsewhere ((x(ixmin1:ixmax1,1) > x_sh) .and. (x(ixmin1:ixmax1,1) < x_sh + thick_sh))
        w(ixmin1:ixmax1,rho_)   = (rho_eq-rho_1)/thick_sh*(x(ixmin1:ixmax1,1)-x_sh) + rho_1
        w(ixmin1:ixmax1,mom(1)) = (v_0-v_1)/thick_sh*(x(ixmin1:ixmax1,1)-x_sh) + v_1
        w(ixmin1:ixmax1,p_)     = (p_eq-p_1)/thick_sh*(x(ixmin1:ixmax1,1)-x_sh) + p_1
