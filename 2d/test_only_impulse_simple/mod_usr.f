...
contains
  ...
  subroutine initvecpot_usr(ixImin1,ixImin2,ixImax1,ixImax2, ixCmin1,ixCmin2,&
    ...
    if (printeo) then
      ! Save the array A to a file
      unit_number = 10  ! File unit number
      open(unit=unit_number, file="vector_potential_A.dat", status="replace", action="write", form="formatted")
      do i = ixCmin1, ixCmax1
         do j = ixCmin2, ixCmax2
         write(unit_number, *) i, j, A(i, j)
         end do
      end do
      close(unit_number)
    end if
