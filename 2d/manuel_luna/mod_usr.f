...
!!!cancellation
  ...
  ! calculate background heating bQ
    ...
    ! end where
      ...
      bQgridspecial1(ixOmin1:ixOmax1,ixOmin2:ixOmax2)=15.d0*bQ0*(abs(A(&
         ixOmin1:ixOmax1,ixOmin2:ixOmax2))/171.73493d0)**(26.d0) !16.d0*bQ0*(abs(A(ixOmin1:ixOmax1,ixOmin2:ixOmax2))/maxval(abs(A(ixOmin1:ixOmax1,ixOmin2:ixOmax2))))**8
      ! end where
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      if(qt>timeimpulse .and. qt<timeimpulse + impulsiveheatingduration) then
        ...
        if(qt>=timeimpulse+impulsiveheatingduration/3.d0 .and. qt < &
           timeimpulse+impulsiveheatingduration*2.d0/3.d0) then
         impulsiveheatingfactor=1.d0+impulsiveheatingfactoramplitude
        endif
        if(qt>=timeimpulse+impulsiveheatingduration*2.d0/3.d0 .and. &
           qt<timeimpulse+impulsiveheatingduration) then
         impulsiveheatingfactor=1.d0+impulsiveheatingfactoramplitude* &
            (-3.d0/impulsiveheatingduration*(qt-timeimpulse-&
            2.d0/3.d0*impulsiveheatingduration)+1.d0)
        endif
      ...
      lQgrid(ixOmin1:ixOmax1,ixOmin2:ixOmax2)=lQ0*( &
         impulsiveheatingfactor*dexp(-(x(ixOmin1:ixOmax1,ixOmin2:ixOmax2,&
...
!    end where
 ...
 subroutine set_output_vars(ixImin1,ixImin2,ixImax1,ixImax2,ixOmin1,ixOmin2,ixOmax1,ixOmax2,qt,w,x)
      ...
      w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,nw+1) = Btotal(ixOmin1:ixOmax1,ixOmin2:ixOmax2,1)
      w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,nw+2) = Btotal(ixOmin1:ixOmax1,ixOmin2:ixOmax2,2)
      w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,nw+3) = Btotal(ixOmin1:ixOmax1,ixOmin2:ixOmax2,3)
