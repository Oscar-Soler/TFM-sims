...
contains
  ...
  subroutine usr_init()
      ...
      unit_temperature    = sqrt(10.0)*1d6
!     unit_velocity = 1 !1.5d7
  ...
  subroutine rm1d_init_one_grid(ixGmin1,ixGmax1,ixmin1,ixmax1,w,x)
    ...
    double precision :: rho0, p0, v0
      ...
      rho0 = 1                  ! 0.5 quizas mejor
      p0 = 1                    ! 0.5 quizas mejor
...
! add local heating lQ
  ...
  !nueva subrutina MLB
    ...
    lQ0= 8.d-3/heatunit !OSP!5.d0*1.d-2/heatunit!MLB 1.d-2/heatunit
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    ! !MLB
      ...
      if(qt>1d0 .and. qt<2.d0) then
         lQgrid(ixOmin1:ixOmax1)=(qt-1d0)/1.d0*lQgrid(ixOmin1:ixOmax1)
      endif

      if(qt>2d0 .and. qt<3.d0) then
         lQgrid(ixOmin1:ixOmax1)=(3d0-qt)/1.d0*lQgrid(ixOmin1:ixOmax1)
      endif
      if(qt>3d0) then
         lQgrid(ixOmin1:ixOmax1)=0.d0
      endif      
