...
!      double precision :: rho0, p0, v0, tau0, T0
!      integer, parameter :: n = 2048
!      integer :: i, ios
!      double precision, allocatable :: xi(:), eta(:), tau(:), p(:), rho(:)
...
!      bQ0=6.0d-5/heatunit       !background heating power density !MLB lo he dividido entre 2, 1d-4
      ...
      !print*, x(ixmin1:ixmax1,1)
      
      call read_data_init()
      call read_data_set(ixmin1,ixmax1,ixGmin1,ixGmax1,x,w,3)
      print*, nw
      print*, x(5:7, 1)
      print*, w(5:7,rho_)
      print*, w(5:7, 1)
      ...
      !if (mype==0) then
         !print*, npe
         !print*, x(ixmin1:ixmax1)
         !print*, size(x)         
         print*, size(w(:,rho_))
         print*, size(w(:,mom(1)))
         print*, size(w(:,p_))
         !print*, w(ixmin1:ixmax1,rho_)
      !end if
       
      
!      allocate(xi(n), eta(n), tau(n), p(n), rho(n))
!      
!! pre-shock status determined only to find tau0, values used for initial state
!! are given in terms of eta, adimensional
!      T0 = 1.1e6
!      rho0 = 4e-16
!      p0 = 6.2e-2
!      v0 = 2.5e7
...
!      
!      !tau0 = 0.25
!! reading of initial conditions in .par file
!
!     open(10, file="initial_conditions.dat", status="old",action="read")
!      do i = 1, n
!         read(10, *, IOSTAT=ios) xi(i), eta(i)
!         !print*, xi(i), eta(i)
!        if (ios /= 0) exit  ! Stop reading if end of file is reached
!     end do
!
!      eta = 1
!
...
!      p = tau/tau0 * rho
!
!!      print*, xprobmin1, xprobmax1
!!      print*, domain_nx1
!!      print*, size(x)
!!      print*, size(eta)
!!      print*, ixmin1, ixmax1
!!     print*, size(w(ixmin1:ixmax1,p_))
!      
!      w(ixmin1:ixmax1,rho_)   = 1!rho
!      w(ixmin1:ixmax1,mom(1)) = 1!eta * rho
!      w(ixmin1:ixmax1,p_)     = 1!p
!
!      
!      !print*, w(ixmin1:ixmax1,p_)
!      !print*, w(ixmin1:ixmax1,rho_)
!      !print*, w(ixmin1:ixmax1,mom(1))
