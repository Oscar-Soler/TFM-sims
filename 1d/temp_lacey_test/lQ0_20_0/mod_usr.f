...
contains
  ...
  subroutine usr_init()
      ...
      unit_length         = 335312549104.5808!=2*v0**4 / rho0 * (mu*mH/kb)**(7/2)*chi_T # cm   !1d9
      unit_numberdensity  = 170818214.23952547 !1d9
      unit_temperature    = 1143000.42908617 !1d6
  ...
  subroutine rm1d_init_one_grid(ixGmin1,ixGmax1,ixmin1,ixmax1,w,x)
    ...
    double precision :: rho0, p0, v0
      ...
      rho0 = 1                 ! 0.5 quizas mejor  quiero 3e-16, con 1 tengo 2.3e-15
      p0 = 1                    ! 0.5 quizas mejor quiero 7e-2, con 1 tengo 3e-1
...
! add local heating lQ
  ...
  !nueva subrutina MLB
    ...
    double precision :: impulsiveheatingfactor,impulsiveheatingduration,&
       ...
       double precision :: heating_half, heating_start, heating_end, heating_1q, heating_3q

    lQ0= 20.d0/heatunit ! 8.d-3/heatunit !OSP!5.d0*1.d-2/heatunit!MLB 1.d-2/heatunit
...
!OSP dimensional time in s for beginning and end of heating
      heating_start = 10d0 / unit_time
      heating_end = 110d0 / unit_time
      heating_half = (heating_end + heating_start)/2
      heating_1q = heating_start + (heating_end-heating_start)*0.25d0
      heating_3q = heating_start + (heating_end-heating_start)*0.75d0
      
      if(qt<heating_start) then
         lQgrid(ixOmin1:ixOmax1)=0.d0
      endif
      if(qt>heating_start .and. qt< heating_1q) then
         lQgrid(ixOmin1:ixOmax1)=(qt-heating_start)/(heating_1q-heating_start)*lQgrid(ixOmin1:ixOmax1)
      endif

      if(qt>heating_3q .and. qt<heating_end) then
         lQgrid(ixOmin1:ixOmax1)=(heating_end-qt)/(heating_end-heating_3q)*lQgrid(ixOmin1:ixOmax1)
      endif
