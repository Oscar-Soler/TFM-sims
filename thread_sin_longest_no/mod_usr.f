    ...
    v_1 = rho_eq/rho_1 * (v_0-v_sh) + v_sh
      ...
      if (sin_mode .eqv. .false.) then
         ...
         where (x(ixmin1:ixmax1,1) < x_sh)
            ...
            w(ixmin1:ixmax1,mom(1)) = v_1
            w(ixmin1:ixmax1,p_)     = p_1
         elsewhere ((x(ixmin1:ixmax1,1) > x_th_ini) .and. (x(ixmin1:ixmax1,1) < x_th_ini+(x_th_fin-x_th_ini)/fraction))
            w(ixmin1:ixmax1,rho_)   = rho_eq + (rho_th-rho_eq) *( (SIN((x(ixmin1:ixmax1,1)-x_th_ini)*pi/((x_th_fin-x_th_ini)/fraction)- pi/2) + 1 )/2)
            w(ixmin1:ixmax1,mom(1)) = v_0
            w(ixmin1:ixmax1,p_)     = p_eq
         elsewhere ((x(ixmin1:ixmax1,1) > x_th_ini+(x_th_fin-x_th_ini)/fraction) .and. (x(ixmin1:ixmax1,1) < x_th_fin-(x_th_fin-x_th_ini)/fraction))
            w(ixmin1:ixmax1,rho_)   = rho_th
            w(ixmin1:ixmax1,mom(1)) = v_0
            w(ixmin1:ixmax1,p_)     = p_eq
         elsewhere ((x(ixmin1:ixmax1,1) > x_th_fin-(x_th_fin-x_th_ini)/fraction) .and. (x(ixmin1:ixmax1,1) < x_th_fin))
            w(ixmin1:ixmax1,rho_)   = rho_eq + (rho_th-rho_eq) *( (SIN((x(ixmin1:ixmax1,1)-(x_th_fin-(x_th_fin-x_th_ini)/fraction))*pi/((x_th_fin-x_th_ini)/fraction)+ pi/2) + 1 )/2)
            w(ixmin1:ixmax1,mom(1)) = v_0
            w(ixmin1:ixmax1,p_)     = p_eq
         elsewhere
            w(ixmin1:ixmax1,rho_)   = rho_eq
